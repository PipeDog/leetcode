...
func deckRevealedIncreasing(deck []int) []int {
	...
	deckLen := len(deck)
	nums := make([]int, deckLen)
	locs := make([]bool, deckLen)
	...
	sort(deck, 0, deckLen-1)

	nextNullIndex := func(start int) int {
		for i := start; i < deckLen; i++ {
			if locs[i] == false {
				return i
			}
		}
	...
	for {
		nums[numIndex] = deck[deckIndex]
		deckIndex++
		locs[numIndex] = true
		
		numIndex = nextNullIndex(nextNullIndex(numIndex) + 1)
